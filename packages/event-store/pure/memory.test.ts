 ...
 *
 * This file opens no connection, which is the point: it sits in `pure/` and
 * runs in `pnpm test`. Its twin, `test/event-store.test.ts`, runs the same
 * contract against a real database in `pnpm test:db`. Neither is the authority
 * on its own — the pair is. A behaviour the fake has and Postgres does not
 * fails there; a behaviour Postgres has and the fake does not fails here.
 */
