 ...
 * (#178).
 *
 * No server: each assertion gets a log in a directory of its own and throws it
 * away. In `integration/` all the same, because a directory of its own is still
 * the filesystem
 ...
 *
 * The contracts are the same files `integration/event-store.test.ts` and
 * `integration/subscribe.test.ts` run against Postgres — not a variant of them
 * — so a behaviour one store has and the other does not is a failing test.
 *
 ...
 * file `integration/queries.test.ts` runs against Postgres — not a variant of it — so
