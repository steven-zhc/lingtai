 ...
 * still open.
 *
 * Whether a daemon should restart *itself* when `main` moves is still open. 0042
...
/** One reason a process started from here could not be accounted for later. */
export interface IdentityRefusal {
  /**
   * Which fact it is, so a caller can treat them differently — and `lingtai
   * restart` does: a dirty worktree can be overridden deliberately, and a
   * commit that is not on the remote, or that could not be checked, cannot.
   */
  what: "unestablished" | "unpushed" | "dirty";
  line: string;
}
 ...
 */
export function identityRefusals(id: Identity): IdentityRefusal[] {
  const at = id.sha ? id.sha.slice(0, 7) : "an unrecorded commit";
  const out: IdentityRefusal[] = [];
