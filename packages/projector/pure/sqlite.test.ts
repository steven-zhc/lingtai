 ...
 *
 * In `pure/` because it needs no server: each assertion gets a database in a
 * directory of its own and throws it away. The contract is the same file
 * `test/projection.test.ts` runs against Postgres — not a variant of it — so a
 * behaviour one store has and the other does not is a failing test rather than
 * a silent divergence.
 *
