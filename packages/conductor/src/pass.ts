 ...
 * tickets and they are filling in a contract this file has already fixed. That
 * is the point of the split: whoever writes `claim`, `admit`, `prepared`,
 ...
 * ([`pass-steps.ts`](pass-steps.ts), `#259`): `bodiesFor` is what a caller hands
 * to `bodies` below, and the four T4b has not reached are still
 * `NOT_BUILT_YET`'s. `NOT_BUILT_YET` stays the default, because *nothing was
 * handed in* must go on doing the thing with no consequences rather than the
 * thing with a GitHub client behind it.
 *
 ...
 * [`pass-steps.ts`](pass-steps.ts) (`#259`) — and this object is still all ten,
 * still the default, and still what runs when a caller hands nothing in. The six
 * rows below therefore describe two different things and say so where they
 * differ: what the step is for, which has not changed, and what *this* body does,
 * which is pass.
 */
...
export interface PassOptions {
   ...
   * **`NOT_BUILT_YET` is the default and stays it**: a pass handed nothing must
   * do the thing with no consequences, not the thing with a GitHub client behind
   * it, so *forgot to pass them* cannot claim a ticket.
   */
