 ...
 * waiting — and **a refusal carrying a criterion both ways**, so that a case
...
describe("the set is the workflow's", () => {
   ...
   *
   * The bar is not *has findings*, it is *carries something the fixer could
   * not have authored* (0038 §2, 0039 §2) — and a refusal with neither buys
   * nothing at either extent today: `decideFix` answers `no-criterion` before
   * it looks at a ceiling, and `decideRestart` refuses *the refusal carried
   * nothing to hold one to*. A rule left in those modules is a rule a judge
  ...
  it("offers nothing but a person when the refusal carries nothing to hold an agent to", () => {
    ...
    // sequence an agent can be asked to make stop happening is one acceptance
    // test, whatever is written beside it.
    expect(
      stepsOnOffer({
        ...EVERYTHING,
        when: "findings",
...
describe("the judge chooses, and cannot widen anything", () => {
   ...
   * `{ ...input, ...input.refusal, offer: stepsOnOffer(input) }` — `input` is
   ...
   * refuses with opinions; a judge, reading only what it is shown, asks for
   * another fixing round — which is reasonable of it and is the unbounded
   * rewriting 0038 §2 is about. `implement` was never on offer, so the answer
