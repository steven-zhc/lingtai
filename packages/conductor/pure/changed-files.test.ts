...
describe("the files a watch is shown", () => {
  it("names where a moved file came from, not only where it went", async () => {
    ...
    await writeFile(
      join(dir, "packages/actions/test/tamper-watch.test.ts"),
    ...
    await git("mv", "packages/actions/test/tamper-watch.test.ts", "doc/tamper-watch.md");
    ...
    expect(names).toContain("packages/actions/test/tamper-watch.test.ts");
