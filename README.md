...
| **Nothing is sent anywhere** | No telemetry, no analytics, no crash reports, no account. Cost and duration are recorded — on your own log, and on `/spend` — and leave your machine for nowhere. Lingtai makes exactly one request of its own: `lingtai upgrade` and `lingtai doctor` ask the GitHub Releases API which version is newest, sending nothing but the question and the version you are on ([0051](doc/decisions/0051-a-version-is-a-directory.md)). Everything else it talks to — GitHub about your repository's issues and branches, the agent about the work you gave it — is the job you asked for. |
