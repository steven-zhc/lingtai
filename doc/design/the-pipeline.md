...
| kind | `tech-debt` |
| blocked by | T5 |
| what | Open the two `KINDS_AT` rows — `build: ["run"]` and `review: ["agent"]` — and take the matching `WHERE_INSTEAD` entries out, so a recipe may declare the build at `build` and the cold reviewer at `review`. The matrix in `doc/reference.md` and `conductor/unit/step-matrix.test.ts` move with them (0059: the document cannot drift from the code without a red test), and the end of it is this machine's own `~/.lingtai/lingtai/recipe.yml` with the two actions at their own steps and `proposed:` holding only judges. |
