...
```
no upcaster for GatesResolved             not free before the reset — see T1
...
| watch out | **This machine's recipe still declares `build` and `review` at `proposed:`, and under the ten steps that costs the fix round rather than buying it.** `~/.lingtai/lingtai/recipe.yml` names `pnpm typecheck && pnpm test` and the cold reviewer as two `proposed:` actions, with `build:` and `review:` empty — which is what `run-once.ts` wanted, because `proposed` *was* the gate. It is not any more: `proposed` is the router, `ARRIVE_AT_THE_ROUTER` does not include it, and a step whose own plugins refused has nothing to route — so a red `pnpm test` now **stops the pass for a person with no round bought**, where before it bought a ~$3.40 fix round. That is 0058 being right rather than the code being wrong. **And the obvious remedy is refused, so it must not be attempted**: no plugin declares itself at `build` or at `review` (each plugin's own `at` since `#261`), so `resolveRecipe` refuses a `run:` at `build` and an `agent:` at `review` by name and `conduct.ts` answers `stage: "recipe"` for every ticket — a machine whose `recipe.yml` was edited that way and then restarted takes nothing at all until the two lines go back. The move is **T5d** below; until it lands, `proposed:` is where the two stay and the first red build of the night is a person's to answer. |
...
| | |
|---|---|
| **T5d** | `build:` and `review:` accept nothing, so the ten-step pass has no home for either |
| kind | `tech-debt` |
| blocked by | T5 |
| what | Add two `at` keys — `build` to `runPlugin` and `review` to `agentPlugin` — and take the matching `WHERE_INSTEAD` entries out, so a recipe may declare the build at `build` and the cold reviewer at `review`. **Since `#261` this is two lines in one file**: there is no `KINDS_AT` row to open and no `doc/reference.md` matrix to keep in step, because a plugin declares the steps it serves (0064 §4). `conductor/unit/step-matrix.test.ts` walks every cell either way. The end of it is this machine's own `~/.lingtai/lingtai/recipe.yml` with the two actions at their own steps and `proposed:` holding only judges. |
| what makes it now | **The refusal's stated reason stopped being true when `#256` landed.** It says *nothing runs a pipeline at `build` yet — the pass that runs it is that plan's next ticket*, and refuses on `#61`'s argument: an action at a step no code reaches would be resolved, recorded in `GatesResolved`, drawn on the board and never called. `runPass` resolves `actionsAt(step, actions)` at **all ten** steps, so that is no longer the shape of the risk — the rows are the only thing left refusing. |
| watch out | **It is the whole of what the recipe's `proposed:` costs today** (T5's second watch-out): `proposed` is the router, a step whose own plugins refused has nothing to route, and a red `pnpm test` declared there reaches a person with no round bought. So this ticket is what makes a red build buy its fix round again, and until it lands nobody should move those lines by hand — the resolve refuses them and the daemon then takes nothing. |
...
- [ ] **No retired name survives** — not in an identifier, a type, an event
      ...
      not* in that same section, and is **larger than what the table tracks**:
      the two test halves alone carry at most 1424 occurrences in 78 files,
      counted by the same rule and more than the ledger holds for `src/`, so
      they are a ticket of their own rather than an afternoon's grep — both
      numbers are the section's and both are checked. `GATE_CARRYING` in
      `packages/domain/unit/upcast.test.ts` and `GateCheckPassed` in
      `apps/board/unit/run-recipe.test.tsx` are two of them and not the two.
      Then `prompts/` — `ticket.md:25`'s *Lingtai runs the gates and the merge
      lane* is in every ticket an agent is given — `scripts/`, and every config
      file, `.lingtai/config.yaml:108`'s `gates:` among them, which nothing
      reads (0046 §3) and still says the word. **A green `#232` is evidence
      for the first clause and a claim about the rest**, and this box ticked on
      the test alone is `#58` again: rendered as resolved, executing nothing
