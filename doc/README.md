...
about processes: one pass from `claim` to `end`, the five points on it, and
where a refusal goes. It draws
[0039](decisions/0039-the-worktree-is-the-whole-of-a-pass.md), and like the
architecture page **everything on it is drawn from code that exists** — it was
ahead of the code for a day. What it replaced, and the constraint that stood in
the way until it was removed, are marked on the page itself. Also in Chinese: **[the-pass.zh.html](the-pass.zh.html)**.
Both are generated by `scripts/the-pass.py`; editing either HTML file loses the
change.
