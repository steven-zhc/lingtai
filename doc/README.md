...
| [`design/the-pipeline.md`](design/the-pipeline.md) | **The ten-step pipeline, whole: the design, the roadmap and the tickets.** A clean cut rather than a migration — the new pass is written beside `run-once.ts` and that file is deleted — in fourteen tickets with their evidence and their traps, plus what a clean cut does *not* buy. Implements 0058, 0060 and 0061; the test split (#224, #225) has landed. |
