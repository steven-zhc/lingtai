...
[0039](decisions/0039-the-worktree-is-the-whole-of-a-pass.md), which is accepted
and **not yet built**, so unlike the architecture page it is ahead of the code —
the two places it is, and the one risk the ADR could not close, are marked on
the page itself. Also in Chinese: **[the-pass.zh.html](the-pass.zh.html)**.
