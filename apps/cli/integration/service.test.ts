...
describe("shutdown", () => {
   ...
   * and what the file lock's queue gives since #193 (`integration/file-lock.test.ts`
   * in `@lingtai/daemon` races it). The command's wait is the real
  ...
  function world(
    ...
    const tick = async (): Promise<void> => {
      ...
      // daemon obeys. That order is `packages/daemon/unit/start.test.ts`'s to
