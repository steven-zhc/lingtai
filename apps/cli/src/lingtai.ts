...
import { releaseCheck, runningVersion } from "./install.ts";
...
const USAGE = `lingtai — event-sourced scheduler for autonomous code agents
  ...
  lingtai version                   the version this lingtai was built as
  lingtai upgrade                   fetch the newest release from GitHub Releases,
                                check its checksum, unpack it beside this one
...
async function main(argv: string[]): Promise<number> {
  ...
  switch (command) {
    ...
    case "version":
      console.log(`lingtai ${runningVersion(import.meta.filename)}`);
      return 0;
    case undefined:
    case "help":
    case "--help":
