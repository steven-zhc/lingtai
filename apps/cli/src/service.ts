 ...
 *
 * ## Two jobs, reported separately (#187)
 *
 ...
 * respawns every thirty seconds. And a board start is confirmed the way a
 * conductor start is: not by the supervisor's exit, but by the board answering
 * on its port.
 *
 * `lingtai restart` passes no `board`, and so reaches only the conductor.
 *
...
import type { PortAnswer } from "./board.ts";
...
/** The board's job (#187), beside the conductor's. */
export const BOARD_LAUNCHD_LABEL = "ai.nextloom.lingtai.board";
export const BOARD_SYSTEMD_UNIT = "lingtai-board.service";

/** One supervised job: what launchd calls it and what systemd does. */
export interface Job {
  label: string;
  unit: string;
}
export const CONDUCTOR_JOB: Job = { label: LAUNCHD_LABEL, unit: SYSTEMD_UNIT };
export const BOARD_JOB: Job = { label: BOARD_LAUNCHD_LABEL, unit: BOARD_SYSTEMD_UNIT };
...
/**
 * The board's LaunchAgent (#187). The conductor's shape without its reasons to
 * be careful: a signal costs a board nothing, so launchd's defaults stand for
 * the plain reason that nothing here needs anything else. The port is not in
 * it — `board start` reads `board.port` from `config.yml` each time it starts.
 */
export function boardLaunchdPlist(inputs: ServiceInputs): ServiceFile {
  const logs = join(stateDir(inputs.env), "logs");
  const env = Object.entries(serviceEnv(inputs))
    .map(([k, v]) => `    <key>${k}</key>\n    <string>${xml(v)}</string>`)
    .join("\n");
...
<dict>
  <key>Label</key>
  <string>${BOARD_LAUNCHD_LABEL}</string>
...
${env}
  ...
  <key>StandardOutPath</key>
  <string>${xml(join(logs, "board.log"))}</string>
  <key>StandardErrorPath</key>
  <string>${xml(join(logs, "board.err"))}</string>
</dict>
</plist>
`;
  return {
    platform: "launchd",
    path: join(home(inputs.env), "Library/LaunchAgents", `${BOARD_LAUNCHD_LABEL}.plist`),
    content,
    logs,
  };
}
...
/**
 * The board's systemd user unit. systemd's default `KillMode` stands: the
 * whole cgroup is the board, `next` included, and there is no agent to spare.
 */
export function boardSystemdUnit(inputs: ServiceInputs): ServiceFile {
  // The same refusals as the conductor's unit, over the same values.
  systemdUnit(inputs);
  const logs = join(stateDir(inputs.env), "logs");
  const content = `# Generated by \`lingtai service install\`. The absolute paths are this machine's —
# regenerate rather than edit, and never commit it.
...
[Service]
Type=simple
ExecStart=${inputs.node} ${join(inputs.root, "apps/cli/src/lingtai.ts")} board start --no-open
WorkingDirectory=${inputs.root}
${Object.entries(serviceEnv(inputs))
  .map(([k, v]) => `Environment=${k}=${v}`)
  .join("\n")}
Restart=always
RestartSec=30
StandardOutput=append:${join(logs, "board.log")}
StandardError=append:${join(logs, "board.err")}
...
`;
  return {
    platform: "systemd",
    path: join(home(inputs.env), ".config/systemd/user", BOARD_SYSTEMD_UNIT),
    content,
    logs,
  };
}
...
export function askSupervisor(platform: ServicePlatform, exec: Exec, uid: number, job: Job = CONDUCTOR_JOB): Supervised {
  if (platform === "launchd") {
...
/** Seconds a board start waits for the board to answer on its port — `next dev` compiles before it does. */
...
export function processRunning(platform: ServicePlatform, lines: readonly string[]): boolean {
...
export async function serviceCommand(args: string[], options: ServiceOptions): Promise<number> {
  ...
  try {
    ...
    boardFile = platform === "launchd" ? boardLaunchdPlist(inputs) : boardSystemdUnit(inputs);
  } catch (err) {
    error((err as Error).message);
    return 1;
  }
  const target = `gui/${uid}/${LAUNCHD_LABEL}`;
  const boardTarget = `gui/${uid}/${BOARD_LAUNCHD_LABEL}`;
   ...
   */
  const unload = async (job: Job = CONDUCTOR_JOB): Promise<boolean> => {
  ...
  const report = async (): Promise<number> => {
    if (options.board) log("the conductor");
    ...
    if (!options.board) return "unread" in answer ? 1 : 0;

    // Its own heading and its own two answers: a board up says nothing about the conductor, nor the other way.
    const board = options.board;
    log("");
    log("the board");
    log(`supervisor  (${platform === "launchd" ? `launchctl, ${boardTarget}` : `systemctl --user, ${BOARD_SYSTEMD_UNIT}`})`);
    const boardAnswer = existsSync(boardFile.path) ? askSupervisor(platform, exec, uid, BOARD_JOB) : null;
    const boardLines =
      boardAnswer === null
        ? [`not installed — ${boardFile.path} does not exist; pnpm lingtai service install writes it`]
        : "unread" in boardAnswer
  ...
  const drain = async (): Promise<number> => {
    ...
    try {
      ...
      log(`the pass in flight finishes first: its agent, the gates and the merge lane. What one may spend is ${await wallLimit()}. It is waiting, not hung.`);
  ...
  const startBoard = async (): Promise<number> => {
    ...
    const url = `http://127.0.0.1:${board.port}`;
    if (answer.loaded && processRunning(platform, answer.lines)) {
      log(`the supervisor is already running the board, so nothing was started — ${describeAnswer(await board.answers(), board.port)}`);
      return 0;
    }
    const before = await board.answers();
    if (before !== "nothing") {
      // A board from a terminal would make the supervised one's failure look like a start.
      error(
        before === "board"
          ? `a board is already on ${board.port} — ${url} — and it is not the supervisor's. lingtai board stop, then pnpm lingtai service start`
          : `${board.port} is held by something that is not a Lingtai board — set board.port in ~/.lingtai/config.yml`,
      );
      return 1;
    }
    const call =
      platform === "systemd"
        ? ["systemctl", "--user", "start", BOARD_SYSTEMD_UNIT]
        : answer.loaded
          ? ["launchctl", "kickstart", boardTarget]
          : ["launchctl", "bootstrap", `gui/${uid}`, boardFile.path];
    if (!run(call)) return 1;
    log(`waiting for the board to answer on ${board.port} — up to ${BOARD_WAIT_POLLS}s`);
    for (let i = 0; i < BOARD_WAIT_POLLS; i++) {
      const now = await board.answers().catch(() => "nothing" as const);
      if (now === "board") {
        log(`the board answers — ${url}`);
        return 0;
      }
      await sleep(1_000);
    }
    error(`the supervisor accepted the board's start, and nothing answered as a board on ${board.port} in ${BOARD_WAIT_POLLS}s — ${join(boardFile.logs, "board.err")} says why`);
    return 1;
  };

  /** A stop, not a drain: the board has no pass to finish. */
  const stopBoard = async (): Promise<number> => {
    ...
    if (!answer.loaded) {
      log(`the supervisor does not have ${platform === "launchd" ? BOARD_LAUNCHD_LABEL : BOARD_SYSTEMD_UNIT} loaded — nothing to stop`);
      return 0;
    }
    const stopped = platform === "launchd" ? await unload(BOARD_JOB) : run(["systemctl", "--user", "stop", BOARD_SYSTEMD_UNIT]);
    if (!stopped) return 1;
    log("stopped the board — it has no pass to finish, so nothing was waited for");
    return 0;
  };
  ...
  const boardHalf = async (): Promise<number> => {
    ...
    switch (verb) {
      ...
      case "shutdown":
        return stopBoard();
      case "restart": {
        // launchd reads a plist only when it loads the job, so a rewritten one is applied by unloading.
        const stopped = await stopBoard();
        return stopped !== 0 ? stopped : startBoard();
      }
      default: {
        const stopped = await stopBoard();
        if (stopped !== 0) return stopped;
        if (platform === "systemd" && existsSync(boardFile.path) && !run(["systemctl", "--user", "disable", BOARD_SYSTEMD_UNIT])) return 1;
        if (!existsSync(boardFile.path)) {
          log(`nothing at ${boardFile.path}`);
          return 0;
        }
...
/**
 * `WALL_LIMIT`, imported when a drain says it. Not at the top: it loads the
 * conductor, which loads the log, and `lingtai board` reads this file's board
 * job on a machine that may have no database configured yet.
 */
async function wallLimit(): Promise<string> {
  return (await import("./wall-limit.ts")).WALL_LIMIT;
}

function describeAnswer(answer: PortAnswer, port: number): string {
  return answer === "board"
    ? `http://127.0.0.1:${port} answers, and it is a Lingtai board`
    : answer === "other"
      ? `something that is not a Lingtai board holds ${port}`
