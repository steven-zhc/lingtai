 ...
 * (#184, [0050](../../../doc/decisions/0050-a-version-is-a-directory.md)).
 *
 *   ~/.lingtai/versions/<v>/
 *     lingtai              the entry — today `lingtai.cjs`, after #185 a binary
...
/** The four artifacts, by what `uname` says. Nothing else is built, so nothing else installs. */
export const PLATFORMS = ["darwin-arm64", "darwin-x64", "linux-x64", "linux-arm64"] as const;

export function artifactName(version: string, platform: string): string {
  return `lingtai-${version}-${platform}.tar.gz`;
}
...
/** `darwin-arm64` and the rest, or the sentence that refuses this machine by name. */
export function platformName(os: string = process.platform, arch: string = process.arch): string | { refused: string } {
...
/**
 * The version this process was built as: the nearest `package.json` named
 * `lingtai`, above the file it was started from.
 *
 * Installed, that is the one beside the entry, which `pnpm release` writes. From
 * a checkout it is the workspace's, which says `0.0.0` and means it.
 */
export function runningVersion(self: string): string {
  let dir = dirname(realpath(self));
  for (;;) {
    const file = join(dir, "package.json");
    if (existsSync(file)) {
      const pkg = JSON.parse(readFileSync(file, "utf8")) as { name?: string; version?: string };
      if (pkg.name === "lingtai" && pkg.version) return pkg.version;
    }
...
export function versionRuns(dir: string, version: string): { ok: true } | { ok: false; said: string } {
  ...
  return ran.status === 0 && said.includes(version) ? { ok: true } : { ok: false, said };
}
...
export async function newestRelease(world: Pick<World, "fetch" | "env" | "self">, timeoutMs = 30_000): Promise<Release> {
  ...
  const res = await world.fetch(`${api}/latest`, {
    headers: { accept: "application/vnd.github+json", "user-agent": `lingtai/${runningVersion(world.self)}` },
...
export async function unpackRelease(
  ...
  const name = artifactName(release.version, platform);
  const artifact = release.assets.find((a) => a.name === name);
  const sums = release.assets.find((a) => a.name === CHECKSUMS);
  if (!artifact) throw new Error(`release ${release.version} has no ${name}`);
  if (!sums) throw new Error(`release ${release.version} has no ${CHECKSUMS}, so ${name} cannot be checked — not installed`);

  world.log(paint.muted(`fetching ${name}`));
  const expected = parseChecksums((await download(world, sums.url)).toString("utf8")).get(name);
  if (!expected) throw new Error(`${CHECKSUMS} for ${release.version} does not list ${name} — not installed`);
  const bytes = await download(world, artifact.url);
  const actual = createHash("sha256").update(bytes).digest("hex");
  if (actual !== expected) {
    throw new Error(`${name} does not match its checksum — expected ${expected}, got ${actual}. Nothing was unpacked`);
  }
  ...
  try {
    ...
    if (!runs.ok) {
      throw new Error(
        `the unpacked lingtai does not run here, so it was not installed — it said: ${runs.said || "nothing"}. ` +
...
export async function installCommand(argv: readonly string[], world: World): Promise<number> {
  ...
  switch (command) {
    case "version":
      world.log(`lingtai ${runningVersion(world.self)}`);
      return 0;
    case "upgrade":
